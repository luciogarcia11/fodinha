...
import {
  createRoom, getRoom, joinRoom, startGame,
  dealRound, updateConfig, disconnectPlayer
} from './game/roomManager';
import { resolveVaza, getForbiddenBet, applyRoundResult, checkGameOver, getCardsForRound } from './game/logic';
...
io.on('connection', (socket) => {
  ...
  socket.on('player:bet', ({ roomId, bet }: { roomId: string; bet: number }) => {
    const state = getRoom(roomId);
    ...
    const isLast = state.bettingOrder.indexOf(socket.id) === state.bettingOrder.length - 1;
    if (isLast) {
      const forbidden = getForbiddenBet(state.bets, state.cardsThisRound);
      if (forbidden !== null && bet === forbidden) {
        socket.emit('room:error', { message: `Você não pode apostar ${forbidden} (seria igual ao número de vazas).` });
        return;
      }
    }
  ...
  socket.on('player:playCard', ({ roomId, cardIndex }: { roomId: string; cardIndex: number }) => {
    const state = getRoom(roomId);
    ...
    if (state.currentTrick.length === activePlayers.length) {
      // Resolve a vaza
      const winnerId = resolveVaza(state.currentTrick, state.config.fdpRule);

      if (winnerId) {
        state.tricksTaken[winnerId] = (state.tricksTaken[winnerId] ?? 0) + 1;
      ...
      if (state.trickNumber > state.cardsThisRound) {
        ...
        const { updatedPlayers, eliminated } = applyRoundResult(
          state.players, state.bets, state.tricksTaken
        );
        ...
        if (gameWinner !== null) {
          ...
          setTimeout(() => {
            ...
            // Rotaciona o pé para o próximo jogador ativo
            const activePlayers = state.players.filter(p => !p.isEliminated);
