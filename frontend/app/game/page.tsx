...
function GameContent() {
  const {
    gameState, myId, placeBet, playCard,
    trickResult, roundEnd, winnerId, clearRoundEnd
  } = useGameContext();
  ...
  function PlayerSlot({ player }: { player: Player }) {
    ...
    return (
      <div className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all min-w-20
      ...
      `}>
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs font-bold text-white/80 truncate max-w-20">{player.name}</span>
          {player.id === myId && <span className="text-xs text-yellow-400">(você)</span>}
        </div>
          ...
          <div className="flex gap-0.5 flex-wrap justify-center max-w-28">
            {player.hand.map((card, i) => (
              <CardComponent
                key={i}
                card={card}
                // Na carta na testa: mostro as cartas dos outros
                // Fora da carta na testa: oculto as cartas dos outros
                hidden={!showOtherCards}
                small
              />
            ))}
          </div>
  ...
  return (
    <main className="min-h-screen flex flex-col relative overflow-hidden select-none">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-black/30 shrink-0">
        ...
        <div className="flex flex-col items-center">
          <span className="text-white text-xs font-bold">
            Rodada {gameState.round} — {gameState.cardsThisRound} carta{gameState.cardsThisRound > 1 ? 's' : ''}
          </span>
          {isCardOnForehead && (
            <span className="text-yellow-300 text-xs animate-pulse">👀 Carta na Testa!</span>
          )}
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-3 p-3 shrink-0">
        {others.map(p => <PlayerSlot key={p.id} player={p} />)}
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-3 px-4">
        ...
        <div className="flex gap-3 flex-wrap justify-center min-h-28 items-center
          ...
          <div className="bg-black/60 rounded-xl px-6 py-2 text-center">
            {trickResult.winnerId
              ? <p className="text-yellow-400 font-bold">
                  {trickResult.winnerId === myId ? '🎉 Você fez a vaza!' :
        ...
        {isBetting && (
          <div className="bg-black/50 rounded-2xl p-4 text-center w-full max-w-sm">
            {isMyBetTurn ? (
              <div>
                <p className="text-white font-bold mb-1">Quantas vazas você vai fazer?</p>
                {isLastBetter && forbiddenBet >= 0 && forbiddenBet <= gameState.cardsThisRound && (
                  ...
                  {gameState.bettingOrder.map(id => {
                    ...
                    return (
                      <div key={id} className="text-xs bg-white/10 rounded px-2 py-1">
                        {p?.name}: {b !== undefined ? <span className="text-yellow-300 font-bold">{b}</span> : '...'}
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="flex gap-2 flex-wrap justify-center px-4">
            {me.hand.map((card, idx) => (
              <CardComponent
                key={idx}
                card={card}
                hidden={isCardOnForehead} // oculta SÓ para o próprio jogador
          ...
          <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-3 text-sm text-white/80">
              ...
              <div className="bg-white/5 rounded-lg p-3">
                ...
                <p>O pé é o último a apostar. Ele <strong className="text-red-400">não pode</strong> fazer a soma das apostas ficar igual ao número de vazas da rodada.</p>
              </div>
      ...
      {showRoundEnd && roundEnd && (
        <div className="absolute inset-0 bg-black/75 flex items-center justify-center z-40 p-4">
          <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-white/30 text-xs text-center mt-3">Próxima rodada em instantes...</p>
          </div>
        </div>
      )}
