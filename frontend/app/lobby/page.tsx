  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center gap-6 p-4">
      ...
      <div className="bg-white/10 rounded-2xl px-8 py-4 text-center">
        ...
        <p className="text-white/40 text-xs mt-1">Compartilhe com seus amigos</p>
      </div>
      ...
      <div className="bg-white/10 rounded-2xl p-4 w-full max-w-sm">
        ...
        <div className="flex flex-col gap-2">
          {gameState.players.map(p => (
            <div key={p.id} className="flex items-center gap-3 bg-white/10 rounded-lg px-3 py-2">
              <span className="text-xl">
                {p.id === gameState.hostId ? '👑' : '🎮'}
              </span>
              <span className="font-medium">{p.name}</span>
              {p.id === myId && (
                <span className="ml-auto text-xs text-yellow-400 font-bold">Você</span>
              )}
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-col gap-3">
            <label className="flex items-center justify-between">
              ...
              <select
                ...
                onChange={e => updateConfig({ livesPerPlayer: Number(e.target.value) })}
              >
                {[1, 2, 3, 4, 5].map(n => (
            ...
            <label className="flex items-center justify-between">
              ...
              <input
                ...
                onChange={e => updateConfig({ cardOnForeheadRule: e.target.checked })}
              />
            </label>
          </div>
        </div>
